...
```bash
sam_rdkitjs$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam_rdkitjs$ sam local start-api
sam_rdkitjs$ curl http://localhost:3000/
```
...
```bash
sam_rdkitjs$ sam logs -n HelloWorldFunction --stack-name sam_rdkitjs --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam_rdkitjs
```
